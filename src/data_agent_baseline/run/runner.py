...
def resolve_run_id(run_id: str | None = None) -> str | None:
    """Validate the run subdirectory name, or return None for a FLAT layout.

    The official evaluation harness mounts ``/output`` and expects predictions
    DIRECTLY at ``/output/task_<id>/...`` with NO nested run folder. A null/blank
    run_id (as in configs/submission.yaml) therefore means "write flat into the
    output root" rather than creating a timestamped subdirectory."""
    if run_id is None:
        return None
...
def create_run_output_dir(output_root: Path, *, run_id: str | None = None) -> tuple[str, Path]:
    ...
    if effective_run_id is None:
        # Flat layout: task dirs go straight into the (already-mounted) output
        # root, so the harness sees /output/task_<id>/ with no extra nesting.
...
def build_video_inspector(config: AppConfig):
...
def build_record_extractor(config: AppConfig):
    ...
    *,
    ...
    if tools is None:
        tools = create_default_tool_registry(
            ...
            video_inspector=build_video_inspector(config),
            record_extractor=build_record_extractor(config),
        )
    ...
    agent = ReActAgent(
        ...
        config=ReActAgentConfig(
            ...
            enable_planner=config.agent.enable_planner,
            enable_answer_verify=config.agent.enable_answer_verify,
            recent_full_steps=config.agent.recent_full_steps,
        ),
    )
...
def _run_single_task_core(
    *,
    ...
    return _asyncio.run(_run_single_task_core_async(
